...
\begin{callout}[title={The short version}]
    Eight months, three PCB revisions, two of them dead on arrival --- and not one of the
    failures was a design error.

    A BOM matcher silently dropped a unit prefix and populated 2.21\,\ohm\ where the
    design called for 2.21\,k\ohm, across every rung of the R-2R ladder. A copy-paste
    during an EDA migration merged the two inputs of an op-amp onto a single net, and ERC
    passed. A filter inductor was chosen on inductance, package, tolerance and
    self-resonance, and its current rating was never read.

    Every one of those passed every check available that was not a physical measurement.
    That is the finding this report exists to record; the working instrument at the end of
    it is almost a by-product.
\end{callout}
...
Raspberry Pi Pico W. It produces sine, square, noise, AM and FM under USB control, is
...
\begin{table}[H]
    ...
    \begin{tabularx}{0.95\linewidth}{@{}>{\bfseries\color{accent}}l X@{}}
        ...
        Supply           & USB, or 2$\times$9\,V batteries                               \\
        Control          & USB CDC serial and desktop UI (\repofile{scripts/awg\_ui.py}) \\
...
\begin{itemize}
    \item \textbf{Every failure that cost a board was invisible to every check that was not
          ...
          review. A fourth, closely related incident --- name and part-number drift in the
...
Dated 26 August 2026, reproduced at full page width overleaf. The embed is vector and
clickable --- scroll or pinch-zoom reads every net and value losslessly, and clicking
the schematic opens \texttt{figures/schematic\_overview.pdf} directly at native size.

\begin{landscape}
    \begin{figure}[H]
        \centering
        \href{figures/schematic_overview.pdf}{%
